...
def main(
    ...
    int, 
    typer.Argument(help="H3 cell resolution to aggregate to, between 0 (largest) and 15 (smallest)")] = 10, 
    output_file: Annotated[
    Paths, 
    typer.Argument(help="File to write output data to (can specify any GDAL-supported format)")]
	): 
	"""
	...
	gvi_mean = gdf_h3.groupby('h3_'+str(cell_resolution)).agg({'gvi_score': ['mean', 'median']})
